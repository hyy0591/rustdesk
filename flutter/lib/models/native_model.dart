...
class PlatformFFI {
  ...
  CscpassistImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libcscpassist.so')
        : isLinux
            ? DynamicLibrary.open('libcscpassist.so')
            : isWindows
                ? DynamicLibrary.open('libcscpassist.dll')
                : isMacOS
                    ? DynamicLibrary.open("liblibcscpassist.dylib")
  ...
  void _startListenEvent(CscpassistImpl CscpassistImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = CscpassistImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
