...
{
  HINSTANCE hInstance = LoadLibraryA("libcscpassist.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libcscpassist.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN cscpassist_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "cscpassist_core_main_args");
  if (!cscpassist_core_main)
  {
    std::cout << "Failed to get cscpassist_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  int args_len = 0;
  char** c_args = cscpassist_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "CscpAssist [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"CscpAssist";
  FUNC_RUSTDESK_GET_APP_NAME get_cscpassist_app_name = (FUNC_RUSTDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_cscpassist_app_name");
  if (get_cscpassist_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_cscpassist_app_name(app_name_buffer, 512) == 0) {
